...
p_load(tidyverse, rio, janitor, RColorBrewer, knitr, 
       ...
       reshape2, R0, shiny, flexdashboard)
...
===
```{r}
selectInput("bairro", label = "Bairro", choices = c("Pedreira", "Marco", "Bengui", ""))
```
...
renderValueBox(
  monitoramento |> 
    ...
    valueBox(icon="virus-covid", color = "warning"))
```
...
renderValueBox(
valueBox(76176, icon="fa-thumbs-down", color = "blue"))
```
...
df2 |> 
    ...
    as.numeric() |>
    round(digits = 2) |> 
      gauge(min = 0, max = 1, gaugeSectors(
  success = c(0, 0.05), warning = c(0.051, 0.07), danger = c(0.071, 1),colors = "warning"))
```
